...
#include <SPI.h>
#include "zTimesNR20.h"
#include "zCalibri28.h"

#define AA_FONT_TIME zTimesNR20
#define AA_FONT_CALI zCalibri28
...
void loopTft()
{
   if (targetTime < millis())
   {
      ...
      if (ss == 0 || initial)
      {
         ...
         tft.drawCentreString("Сауна", 70, 48, 2); // Next size up font 2
      ...
      else
      {
         ...
         tft.fillRect(0, 74, 160, 20, TFT_BLACK);
         tft.setTextColor(colour);
         tft.drawRightString("Душ", 75, 74, 4); // Right justified string drawing to x position 75
         String scolour = String(colour, HEX);
         scolour.toUpperCase();
         char buffer[20];
         scolour.toCharArray(buffer, 20);
         tft.drawString(buffer, 82, 74, 4);
      }
   }
}